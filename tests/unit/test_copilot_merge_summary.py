...
def test_workflow_gates_sync_on_tests_and_copilot_without_exposing_tracker_credentials():
    ...
    assert "types: [opened, reopened, synchronize, closed]" in before
    assert "  unit-tests:\n" in before
    assert "github.event.action != 'closed' || github.event.pull_request.merged == true" in before
    assert "github.ref || 'main'" in before
    ...
    assert "if: github.event_name == 'workflow_dispatch' || github.event.pull_request.merged == true" in after
